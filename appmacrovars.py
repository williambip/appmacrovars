...
# ### cria as sessions states para os botoes
# if 'dict_vars' not in st.session_state:
#     st.session_state['dict_vars'] = {}
# if 'var_name' not in st.session_state:
#     st.session_state['var_name'] = ''
# if 'var_value' not in st.session_state:
#     st.session_state['var_value'] = 0
...
#         slugified_var_name = slugify(var_name)
#         st.session_state['dict_vars'][slugified_var_name] = int(var_value)
#         #st.session_state['dict_vars'][var_name] = int(var_value)
#         st.success(f'Variável "{var_name}" com valor {var_value} adicionada.')
#         # Reset var_name and var_value in session state to clear the input fields
#         st.session_state['var_name'] = ''
#         st.session_state['var_value'] = 0
#     else:
#         st.error('Por favor, insira tanto o nome da variável quanto o valor.')
...
# # Button to add the variable, linked to the add_variable function
# st.button('Adicionar Variável', on_click=add_variable)
...
# # Option to reset variables
# if st.button('Redefinir Variáveis'):
#     st.session_state['dict_vars'] = {}
#     st.success('Variáveis redefinidas.')
#     st.session_state['data_collected'] = False

# # Proceed with data collection
# if st.button('Prosseguir com Coleta de Dados'):
#     if not st.session_state['dict_vars']:
#         st.error('Nenhuma variável adicionada. Por favor, adicione pelo menos uma variável.')
#     else:
#         #st.write(f'Data Início: {data_inicio.strftime("%d/%m/%Y")}')
#         #st.write(f'Data Fim: {data_fim.strftime("%d/%m/%Y")}')
#         #st.write('Variáveis Selecionadas:')
#         #st.write(st.session_state['dict_vars'])

#         # Instantiate your class and use the variables
#         try:
#             data_collector = CollectDataBacen(
#                 dict_vars=st.session_state['dict_vars'],
#                 start_date=data_inicio.strftime('%d/%m/%Y'),
...
#             st.session_state['data_collected'] = True  # Set the flag to indicate data has been collected
#         except Exception as e:
#             st.error(f'Ocorreu um erro: {e}')
#             st.session_state['data_collected'] = False
...
#     # Plotting the data
#     st.subheader('Gráfico de Linhas das Variáveis Econômicas')
#     st.line_chart(st.session_state['df'], height=600, width=1000)
#     ### download
#     st.subheader('Download dos Dados')
